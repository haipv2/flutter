...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _shareImage() async {
    try {
      final ByteData bytes = await rootBundle.load('images/bg.png');
      await Share.file(
  ...
  Future<void> _shareImages() async {
    try {
      final ByteData bytes1 = await rootBundle.load('images/bg.png');
      final ByteData bytes2 = await rootBundle.load('images/smile.png');
  ...
  Future<void> _shareCSV() async {
    try {
      final ByteData bytes = await rootBundle.load('images/addresses.csv');
      await Share.file(
  ...
  Future<void> _shareMixed() async {
    try {
      final ByteData bytes1 = await rootBundle.load('images/bg.png');
      final ByteData bytes2 = await rootBundle.load('images/smile.png');
      final ByteData bytes3 = await rootBundle.load('images/addresses.csv');

      await Share.files(
          'esys images',
          {
            'bg.png': bytes1.buffer.asUint8List(),
            'smile.png': bytes2.buffer.asUint8List(),
  ...
  Future<void> _shareSound() async {
    try {
      final ByteData bytes = await rootBundle.load('images/cat.mp3');
      await Share.file(
